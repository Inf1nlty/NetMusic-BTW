...
public final class PlayerVipCookieStore {
    ...
    public static CookiePair getCookies(EntityPlayer player) {
        if (player == null) {
            return CookiePair.EMPTY;
        }
        String playerKey = resolvePlayerKey(player);
        CookiePair pair = getCookies(playerKey);
        if (!isEmptyPair(pair)) {
            return pair;
        }

        String legacyUuidKey = resolveLegacyUuidKey(player);
        if (StringUtils.isBlank(legacyUuidKey) || legacyUuidKey.equals(playerKey)) {
            return pair;
        }
        CookiePair legacy = getCookies(legacyUuidKey);
        if (!isEmptyPair(legacy)) {
            // Migrate legacy UUID-keyed cookie to name-keyed entry.
            setCookies(playerKey, legacy.neteaseCookie, legacy.qqCookie);
            return legacy;
        }
        return pair;
    }
    ...
    public static void setCookies(EntityPlayer player, String neteaseCookie, String qqCookie) {
        setCookies(resolvePlayerKey(player), neteaseCookie, qqCookie);
    }
    ...
    public static boolean hasVipCookieForUrl(EntityPlayer player, String songUrl) {
        CookiePair pair = getCookies(player);
        return hasVipCookieForUrl(songUrl, pair.neteaseCookie, pair.qqCookie);
    }
    ...
    public static String resolvePlayerKey(EntityPlayer player) {
        if (player == null) {
            return null;
        }
        ...
        return resolveLegacyUuidKey(player);
    }

    public static String resolveLegacyUuidKey(EntityPlayer player) {
        if (player == null) {
            return null;
        }
    ...
    private static boolean isEmptyPair(CookiePair pair) {
        return pair == null
                || (StringUtils.isBlank(pair.neteaseCookie) && StringUtils.isBlank(pair.qqCookie));
    }
